...
- Topic 4 now carries the next repair/population backlog. `replaceOne()` is study-ready at `5 Easy + 2 Medium` but remains the ordered target until it reaches the `5 Easy + 5 Medium` population inventory.
...
- **Current execution note**: Topic 4 is the active ordered target. `replaceOne()` has 47 repairable quarantines, no hard-delete candidates, and a remaining population deficit of 3 Medium. The overnight runner is local-only for long repair/populate runs.
...
- Continue Topic 4 from `replaceOne()`.
- Do not advance to `updateOne()` until the selector advances.
