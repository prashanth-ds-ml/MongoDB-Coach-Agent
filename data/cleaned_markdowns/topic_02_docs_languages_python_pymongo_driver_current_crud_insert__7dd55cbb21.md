...
| --- | --- |
| `bypass_document_validation` | If set to `True`, allows the write to opt out of document-level validation.Defaults to `False`. |
...
| Property | Description |
| --- | --- |
| `ordered` | If set to `True`, the driver sends documents to the server in the order provided. If an error occurs, the driver and server cancel all remaining insert operations.Defaults to `True`. |
### Example
...
The NotRequired class is available only in Python 3.11 and later. To use `NotRequired` in earlier versions of Python, install the typing_extensions package instead.

## Additional Information

For runnable code examples of inserting documents with PyMongo, see CRUD Operations.
