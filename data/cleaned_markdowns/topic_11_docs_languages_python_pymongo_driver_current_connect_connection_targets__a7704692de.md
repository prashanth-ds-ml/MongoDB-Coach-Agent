...
PyMongo evenly load balances operations across deployments that are reachable within the client's `localThresholdMS` value. To learn more about how PyMongo load balances operations across multiple MongoDB deployments, see the Customize Server Selection guide.
