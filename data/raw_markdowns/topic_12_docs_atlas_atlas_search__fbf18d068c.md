...
You can use MongoDB Search for fine-grained text indexing and querying of data on your cluster. MongoDB Search provides several kinds of [text analyzers](/docs/atlas/atlas-search/analyzers#std-label-analyzers-ref), a rich [query language](/docs/atlas/atlas-search/query-ref#std-label-query-syntax-ref) to create complex search logic, customizable score-based results ranking, and advanced search features for your applications like autocomplete, pagination, and faceting.
...
- **Search-as-you-type:** To predict words with increasing accuracy as users enter characters in your application's search field, you can use the MongoDB Search [autocomplete](/docs/atlas/atlas-search/operators-collectors/autocomplete#std-label-autocomplete-ref) operator to predict and return results for partial words. To learn more, see [How to Run Autocomplete and Partial Match MongoDB Search Queries.](/docs/atlas/atlas-search/tutorial/partial-match#std-label-partial-match-tutorial)

- **Faceted Search:** To enable users of your application to narrow down search results through the use of filters, you can use the MongoDB Search [`facet` (MongoDB Search Operator)](/docs/atlas/atlas-search/operators-collectors/facet#std-label-fts-facet-ref) collector to build facets that group results by values or ranges in the faceted fields. To learn more, see [How to Use Facets with MongoDB Search.](/docs/atlas/atlas-search/tutorial/facet-tutorial#std-label-facet-tutorial)

- **Paginated Results:** To group pages of results and implement functions like "Next Page" and "Previous Page", you can use the MongoDB Search `searchSequenceToken` with `searchAfter` and `searchBefore` options to traverse pages in-order and jump across pages. To learn more, see [How to Paginate the Results.](/docs/atlas/atlas-search/paginate-results#std-label-fts-paginate-results)
...
MongoDB Search queries take the form of an [aggregation pipeline stage](https://www.mongodb.com/docs/manual/aggregation/). MongoDB Search provides [`$search`](/docs/atlas/atlas-search/aggregation-stages/search#mongodb-pipeline-pipe.-search) and [`$searchMeta`](/docs/atlas/atlas-search/aggregation-stages/searchMeta#mongodb-pipeline-pipe.-searchMeta) stages, which you can use with other [aggregation pipeline stages](https://www.mongodb.com/docs/manual/aggregation/) in your query pipeline. MongoDB Search provides query [operators](/docs/atlas/atlas-search/operators-and-collectors#std-label-operators-ref) and [collectors](/docs/atlas/atlas-search/operators-and-collectors#std-label-collectors-ref) that you can use inside these aggregation pipeline stages.

To learn more, see [Queries and Indexes.](/docs/atlas/atlas-search/searching#std-label-fts-about-queries)
...
You can create a MongoDB Search index on a single field or on multiple fields by using [static mappings](/docs/atlas/atlas-search/define-field-mappings#std-label-static-dynamic-mappings). Alternatively, you can enable [dynamic mappings](/docs/atlas/atlas-search/define-field-mappings#std-label-static-dynamic-mappings) to automatically index all the dynamically indexable fields in your documents. You can create MongoDB Search indexes on polymorphic data and embedded documents, or for specific use-cases like search-as-you-type or faceted search.

To learn more, see [Supported Clients.](/docs/atlas/atlas-search/manage-indexes#std-label-fts-manage-indexes)
...
MongoDB Search provides some built-in [analyzers](/docs/atlas/atlas-search/analyzers#std-label-analyzers-ref). You can also create your own [custom analyzer](/docs/atlas/atlas-search/analyzers/custom#std-label-custom-analyzers). You can specify alternate analyzers using [multi](/docs/atlas/atlas-search/analyzers/multi#std-label-ref-multi-analyzers) analyzer.

To learn more, see [Process Data with Analyzers.](/docs/atlas/atlas-search/analyzers#std-label-analyzers-ref)
...
To learn more, see [Score Documents.](/docs/atlas/atlas-search/scoring#std-label-scoring-ref)

## Next Steps

For a hands-on experience creating MongoDB Search indexes and running MongoDB Search queries against sample data, try the [MongoDB Search Quick Start.](/docs/atlas/atlas-search/tutorial#std-label-fts-tutorial-ref)
