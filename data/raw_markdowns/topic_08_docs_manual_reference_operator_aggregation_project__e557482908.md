...
`$project`You can use `$project` for deployments hosted in the following environments:
...
The [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage has the following prototype form:
...
The [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) specifications have the following forms:
...
To exclude a field conditionally, use the [`REMOVE`](https://www.mongodb.com/docs/reference/aggregation-variables/#mongodb-variable-variable.REMOVE) variable instead. For details, see [Exclude Fields Conditionally](https://www.mongodb.com/docs/reference/operator/aggregation/project/#std-label-remove-var).
...
If the expression evaluates to `$$REMOVE`, the field is excluded in the output. For details, see [Exclude Fields Conditionally](https://www.mongodb.com/docs/reference/operator/aggregation/project/#std-label-remove-var).
...
To exclude fields, you can also use the [`$unset`](https://www.mongodb.com/docs/reference/operator/aggregation/unset/#mongodb-pipeline-pipe.-unset) stage.
...
- The `_id` field is, by default, included in the output documents. To include any other fields from the input documents in the output documents, you must explicitly specify the inclusion in [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project).

- If you specify an inclusion of a field that does not exist in the document, [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) ignores that field inclusion and does not add the field to the document.

### `_id` Field

By default, the `_id` field is included in the output documents. To exclude the `_id` field from the output documents, you must explicitly specify the suppression of the `_id` field in [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project).
...
See also the [`$unset`](https://www.mongodb.com/docs/reference/operator/aggregation/unset/#mongodb-pipeline-pipe.-unset) stage to exclude fields.

#### Exclude Fields Conditionally

You can use the variable [`REMOVE`](https://www.mongodb.com/docs/reference/aggregation-variables/#mongodb-variable-variable.REMOVE) in aggregation expressions to conditionally suppress a field. For an example, see [Conditionally Exclude Fields](https://www.mongodb.com/docs/reference/operator/aggregation/project/#std-label-remove-example).

### Add New Fields or Reset Existing Fields

MongoDB also provides [`$addFields`](https://www.mongodb.com/docs/reference/operator/aggregation/addFields/#mongodb-pipeline-pipe.-addFields) to add new fields to the documents.

To add a new field or to reset the value of an existing field, specify the field name and set its value to some expression. For more information on expressions, see [Expressions](https://www.mongodb.com/docs/reference/mql/expressions/#std-label-aggregation-expressions).

#### Literal Values

To set a field value directly to a numeric or boolean literal, as opposed to setting the field to an expression that resolves to a literal, use the [`$literal`](https://www.mongodb.com/docs/reference/operator/aggregation/literal/#mongodb-expression-exp.-literal) operator. Otherwise, [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) treats the numeric or boolean literal as a flag for including or excluding the field.
...
The [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage supports using the square brackets `[]` to directly create new array fields. If you specify array fields that do not exist in a document, the operation substitutes `null` as the value for that field. For an example, see [Project New Array Fields](https://www.mongodb.com/docs/reference/operator/aggregation/project/#std-label-example-project-new-array-fields).

You cannot use an array index with the [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage.

### Embedded Document Fields

When projecting or adding/resetting a field within an embedded document, you can either use [dot notation](https://www.mongodb.com/docs/reference/glossary/#std-term-dot-notation), as in
...
The following [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage fails with a `Path collision` error because it attempts to project both the embedded `contact` document and the `contact.address.country` field:

```javascript
{ $project: { contact: 1, "contact.address.country": 1 } }
```

The error occurs regardless of the order in which the parent document and embedded field are specified. The following [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) fails with the same error:
...
MongoDB returns an error if the [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage is passed an empty document.
...
The following [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage includes only the `_id`, `title`, and `rated` fields:
...
MongoDB includes the `_id` field by default. To exclude the `_id` field from the [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage, set the `_id` field to `0` in the projection document.

The following [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage excludes the `_id` field but includes the `title` and `rated` fields:
...
The following [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage excludes the `rated` field from the output:
...
Alternatively, use the [`$unset`](https://www.mongodb.com/docs/reference/operator/aggregation/unset/#mongodb-pipeline-pipe.-unset) stage to exclude fields.

### Exclude Fields from Embedded Documents

The following [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage excludes the `imdb.id` and `type` fields from the output:
...
You can use the variable [`REMOVE`](https://www.mongodb.com/docs/reference/aggregation-variables/#mongodb-variable-variable.REMOVE) in aggregation expressions to conditionally suppress a field.

The following [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage uses the [`REMOVE`](https://www.mongodb.com/docs/reference/aggregation-variables/#mongodb-variable-variable.REMOVE) variable to exclude the `imdb.votes` field if it equals `null` or is an empty string:
...
To see an example using `$$REMOVE` in an `$addFields` stage, see [Remove Fields](https://www.mongodb.com/docs/reference/operator/aggregation/addFields/#std-label-addFields-remove-example).

### Include Specific Fields from Embedded Documents

Documents in the `movies` collection contain an embedded `imdb` document with the `rating`, `votes`, and `id` fields. To include only the `rating` field from the `imdb` document, use [dot notation](https://www.mongodb.com/docs/reference/glossary/#std-term-dot-notation):
...
The following [`$project`](https://www.mongodb.com/docs/reference/operator/aggregation/project/#mongodb-pipeline-pipe.-project) stage adds the `leadActor` and `releaseYear` fields:
...
The preceding example uses `BsonDocument` objects because the .NET/C# driver doesn't provide a builder to conditionally exclude fields. Other MongoDB language drivers might support this feature. See the [MongoDB driver documentation](https://www.mongodb.com/docs/drivers/) for more information.
...
To conditionally exclude a field, assign conditional logic that includes the variable [`REMOVE`](https://www.mongodb.com/docs/reference/aggregation-variables/#mongodb-variable-variable.REMOVE) to the field name.
...
For more information and detailed explanations of common aggregation tasks, see the [Complete Aggregation Pipeline Tutorials](https://www.mongodb.com/docs/tutorial/aggregation-complete-examples/#std-label-aggregation-complete-examples).

To learn more about the methods on this page, see the following guides:

- [`$eq`](https://www.mongodb.com/docs/reference/operator/aggregation/eq/#mongodb-expression-exp.-eq)

- [`$cond`](https://www.mongodb.com/docs/reference/operator/aggregation/cond/#mongodb-expression-exp.-cond)

- [`$arrayElemAt`](https://www.mongodb.com/docs/reference/operator/aggregation/arrayElemAt/#mongodb-expression-exp.-arrayElemAt)

- [`$concatArrays`](https://www.mongodb.com/docs/reference/operator/aggregation/concatArrays/#mongodb-expression-exp.-concatArrays)

- [`REMOVE`](https://www.mongodb.com/docs/reference/aggregation-variables/#mongodb-variable-variable.REMOVE)
