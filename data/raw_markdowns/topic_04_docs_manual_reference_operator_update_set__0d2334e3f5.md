...
The following page refers to the update operator [`$set`](https://www.mongodb.com/docs/reference/operator/update/set/#mongodb-update-up.-set). For the aggregation stage, see [`$set`](https://www.mongodb.com/docs/reference/operator/aggregation/set/#mongodb-pipeline-pipe.-set).

`$set`
The [`$set`](https://www.mongodb.com/docs/reference/operator/update/set/#mongodb-update-up.-set) operator replaces the value of a field with the specified value.

## Compatibility

`$set`You can use `$set` for deployments hosted in the following environments:
...
The [`$set`](https://www.mongodb.com/docs/reference/operator/update/set/#mongodb-update-up.-set) operator expression has the following form:
...
If the field does not exist, [`$set`](https://www.mongodb.com/docs/reference/operator/update/set/#mongodb-update-up.-set) will add a new field with the specified value, provided that the new field does not violate a type constraint. If you specify a dotted path for a non-existent field, [`$set`](https://www.mongodb.com/docs/reference/operator/update/set/#mongodb-update-up.-set) will create the embedded documents *as needed* to fulfill the dotted path to the field.

If you specify multiple field-value pairs, [`$set`](https://www.mongodb.com/docs/reference/operator/update/set/#mongodb-update-up.-set) will update or create each field.

Starting in MongoDB 5.0, [`mongod`](https://www.mongodb.com/docs/reference/program/mongod/#mongodb-binary-bin.mongod) no longer raises an error when you use an update operator like [`$set`](https://www.mongodb.com/docs/reference/operator/update/set/#mongodb-update-up.-set) with an empty operand expression ( `{ }` ). An empty update results in no changes and no [oplog](https://www.mongodb.com/docs/reference/glossary/#std-term-oplog) entry is created (meaning that the operation is a no-op).
...
- **Efficient Oplog Entries**: `$set` optimizes replication by writing only the updated fields to the [oplog](https://www.mongodb.com/docs/reference/glossary/#std-term-oplog) instead of the entire document. This process reduces the size of oplog entries and allows nodes to replicate changes more efficiently.
...
For the document matching the criteria `_id` equal to `100`, the following operation uses the [`$set`](https://www.mongodb.com/docs/reference/operator/update/set/#mongodb-update-up.-set) operator to update the value of the `quantity` field, `details` field, and the `tags` field.
...
For additional update operators for arrays, see [Array Update Operators](https://www.mongodb.com/docs/reference/operator/update-array/).
