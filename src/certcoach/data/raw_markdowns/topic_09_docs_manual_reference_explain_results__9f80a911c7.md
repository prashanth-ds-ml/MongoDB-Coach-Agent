...
To return information on [query plans](https://www.mongodb.com/docs/core/query-plans/#std-label-query-plans-query-optimization) and execution statistics of the query plans, MongoDB provides the following methods:

- [`db.collection.explain()`](https://www.mongodb.com/docs/reference/method/db.collection.explain/#mongodb-method-db.collection.explain)

- [`cursor.explain()`](https://www.mongodb.com/docs/reference/method/cursor.explain/#mongodb-method-cursor.explain)

To learn about important explain result fields and how to interpret them, see [Interpret Explain Plan Results](https://www.mongodb.com/docs/tutorial/analyze-query-plan/#std-label-interpret-explain-plan).
...
The `explain` results present the query plans as a tree of stages. The output structure can differ based on which query engine the operation uses. Operations can use the classic query engine or the [slot-based execution query engine](https://www.mongodb.com/docs/reference/sbe/#std-label-sbe-landing).
...
[`explain.queryPlanner`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.queryPlanner) information details the plan selected by the [query optimizer](https://www.mongodb.com/docs/core/query-plans/#std-label-read-operations-query-optimization).
...
Contains information on the selection of the query plan by the [query optimizer](https://www.mongodb.com/docs/core/query-plans/#std-label-read-operations-query-optimization).

`explain.queryPlanner.namespace`
A string that specifies the [namespace](https://www.mongodb.com/docs/reference/glossary/#std-term-namespace) with the names of the database and the collection accessed by the query. The namespace has the format `<database>.<collection>`.

`explain.queryPlanner.indexFilterSet`
A boolean that specifies whether MongoDB applied an [index filter](https://www.mongodb.com/docs/core/query-plans/#std-label-index-filters) for the [plan cache query shape](https://www.mongodb.com/docs/reference/glossary/#std-term-plan-cache-query-shape).
...
To add query settings and explore examples, which include `explain()` output with `querySettings`, see [`setQuerySettings`](https://www.mongodb.com/docs/reference/command/setQuerySettings/#mongodb-dbcommand-dbcmd.setQuerySettings).

`explain.queryPlanner.planCacheShapeHash`
Starting in MongoDB 8.0, the existing `queryHash` field is duplicated in a new field named `planCacheShapeHash`. If you're using an earlier MongoDB version, you'll only see the `queryHash` field. Future MongoDB versions will remove the deprecated `queryHash` field, and you'll need to use the `planCacheShapeHash` field instead.

A hexadecimal string that represents the hash of the [plan cache query shape](https://www.mongodb.com/docs/reference/glossary/#std-term-plan-cache-query-shape) and is dependent only on the plan cache query shapes. `planCacheShapeHash` can help identify slow queries (including the query filter of write operations) with the same plan cache query shape.
...
For more information on `planCacheShapeHash` and `planCacheKey`, see [planCacheShapeHash and planCacheKey](https://www.mongodb.com/docs/core/query-plans/#std-label-query-hash-plan-cache-key).

`explain.queryPlanner.planCacheKey`
A hash of the key for the plan cache entry associated with the query.

Unlike the [`explain.queryPlanner.planCacheShapeHash`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.queryPlanner.planCacheShapeHash), the [`explain.queryPlanner.planCacheKey`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.queryPlanner.planCacheKey) is a function of both the plan cache query shape and the currently available indexes for that shape. Specifically, if indexes that can support the query shape are added or dropped, the `planCacheKey` value may change but the `planCacheShapeHash` value wouldn't change.

For more information on `planCacheShapeHash` and `planCacheKey`, see [planCacheShapeHash and planCacheKey](https://www.mongodb.com/docs/core/query-plans/#std-label-query-hash-plan-cache-key).

`explain.queryPlanner.optimizationTimeMillis`
Time in milliseconds the query planner spent on query optimization. This result doesn't include time spent optimizing inner [`$lookup`](https://www.mongodb.com/docs/reference/operator/aggregation/lookup/#mongodb-pipeline-pipe.-lookup) queries.
...
A document that details the plan selected by the [query optimizer](https://www.mongodb.com/docs/core/query-plans/).
...
An array of documents describing the child stages. Child stages provide the documents or index keys to the parent stage. The field is present *if* the parent stage has multiple child nodes. For example, stages for [$or expressions](https://www.mongodb.com/docs/reference/explain-results/#std-label-explain-output-or-expression) might consume input from multiple sources.
...
A document that details the plan selected by the [query optimizer](https://www.mongodb.com/docs/core/query-plans/). MongoDB presents the plan as a tree of stages.

This document appears if the query used the [slot-based execution query engine](https://www.mongodb.com/docs/reference/sbe/#std-label-sbe-landing).
...
See [`explain.queryPlanner.winningPlan.inputStage`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.queryPlanner.winningPlan.inputStage).
...
Starting in MongoDB 8.0, if a query uses [block processing](https://www.mongodb.com/docs/core/timeseries/timeseries-querying/#std-label-timeseries-querying-block-processing), `block_to_row` and `ts_bucket_to_cellblock` appear in the `stages` output.
...
Starting in MongoDB 8.0, rejected query plans only contain the `find` portion of the query. In previous versions, rejected plans can contain aggregation stages like [`$group`](https://www.mongodb.com/docs/reference/operator/aggregation/group/#mongodb-pipeline-pipe.-group). Those aggregation stages aren't used by the query planner to choose the winning plan, so the `rejectedPlans` field only contains the portion of the query that was used to choose the winning plan.

### `executionStats`

The returned [`explain.executionStats`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.executionStats) information details the execution of the winning plan. In order to include `executionStats` in the results, you must run the explain in either:

- [executionStats](https://www.mongodb.com/docs/reference/method/db.collection.explain/#std-label-explain-method-executionStats) or

- [allPlansExecution](https://www.mongodb.com/docs/reference/method/db.collection.explain/#std-label-explain-method-allPlansExecution) verbosity mode. Use `allPlansExecution` mode to include partial execution data captured during [plan selection](https://www.mongodb.com/docs/core/query-plans/#std-label-query-plans-query-optimization).
...
Number of documents returned by the winning query plan. [`nReturned`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.executionStats.nReturned) corresponds to the `n` field returned by `cursor.explain()` in earlier versions of MongoDB.
...
Number of index entries scanned. [`explain.executionStats.totalKeysExamined`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.executionStats.totalKeysExamined) corresponds to the `nscanned` field returned by `cursor.explain()` in earlier versions of MongoDB.
...
[`explain.executionStats.totalDocsExamined`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.executionStats.totalDocsExamined) refers to the total number of documents examined and *not* to the number of documents returned. For example, a stage can examine a document in order to apply a filter. If the document is filtered out, then it has been examined but will not be returned as part of the query result set.

If a document is examined multiple times during query execution, [`explain.executionStats.totalDocsExamined`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.executionStats.totalDocsExamined) counts each examination. That is, [`explain.executionStats.totalDocsExamined`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.executionStats.totalDocsExamined) is *not* a count of the total number of *unique* documents examined.
...
Each `inputStage` can have another `inputStage` as a field. See [Explain Output Structure](https://www.mongodb.com/docs/reference/explain-results/#std-label-explain-output-structure).
...
<td headers="Applicable%20Stages">
`COLLSCAN`, `FETCH`
...
</table>`explain.executionStats.allPlansExecution`
Contains *partial* execution information captured during the [plan selection phase](https://www.mongodb.com/docs/core/query-plans/#std-label-query-plans-query-optimization) for both the winning and rejected plans. The field is present only if `explain` runs in `allPlansExecution` verbosity mode.
...
MongoDB 8.2 removes `profileOperationResourceConsumptionMetrics`. Therefore, code depending on [operationMetrics](https://www.mongodb.com/docs/reference/explain-results/#std-label-executionStats) does not work.
...
For sharded collections, `explain` returns the `serverInfo` for each accessed shard, and a top-level `serverInfo` object for the [`mongos`](https://www.mongodb.com/docs/reference/program/mongos/#mongodb-binary-bin.mongos).
...
The [explain results](https://www.mongodb.com/docs/reference/explain-results/#std-label-explain-results) can include execution statistics for queries that use a [`$lookup`](https://www.mongodb.com/docs/reference/operator/aggregation/lookup/#mongodb-pipeline-pipe.-lookup) pipeline stage. To include those execution statistics, you must run the explain operation in one of these execution verbosity modes:

- [executionStats](https://www.mongodb.com/docs/reference/method/db.collection.explain/#std-label-explain-method-executionStats)

- [allPlansExecution](https://www.mongodb.com/docs/reference/method/db.collection.explain/#std-label-explain-method-allPlansExecution)

The following fields are included in the explain results for a [`$lookup`](https://www.mongodb.com/docs/reference/operator/aggregation/lookup/#mongodb-pipeline-pipe.-lookup) query:
...
To see the descriptions for the fields in the `$lookup` section, see the [`$lookup`](https://www.mongodb.com/docs/reference/operator/aggregation/lookup/#mongodb-pipeline-pipe.-lookup) page.
...
Number of times a collection scan occurred during query execution. During a collection scan, each document in a collection is compared to the query predicate. Collection scans occur if no appropriate [index](https://www.mongodb.com/docs/indexes/#std-label-indexes) exists that [covers](https://www.mongodb.com/docs/core/query-optimization/#std-label-indexes-covered-queries) the query.
...
The explain results includes [execution statistics](https://www.mongodb.com/docs/reference/explain-results/#std-label-executionStats) for queries that use a [`$search`](https://www.mongodb.com/docs/atlas/atlas-search/aggregation-stages/search/#mongodb-pipeline-pipe.-search), [`$searchMeta`](https://www.mongodb.com/docs/atlas/atlas-search/aggregation-stages/searchMeta/#mongodb-pipeline-pipe.-searchMeta), or [`$vectorSearch`](https://www.mongodb.com/docs/vector-search/query/aggregation-stages/vector-search-stage/#mongodb-pipeline-pipe.-vectorSearch) pipeline stage. To include execution statistics for search queries, run the `explain` command in one of the following execution verbosity modes:
...
- [Explain Search Results](https://www.mongodb.com/docs/atlas/atlas-search/explain/#std-label-explain-ref)

- [Explain MongoDB Vector Search Results](https://www.mongodb.com/docs/vector-search/query/explain/#std-label-avs-explain-ref)
...
Starting in MongoDB 5.3, if the query planner selects a [clustered index](https://www.mongodb.com/docs/reference/method/db.createCollection/#std-label-db.createCollection.clusteredIndex) for a [clustered collection](https://www.mongodb.com/docs/core/clustered-collections/#std-label-clustered-collections) and the query contains bounds that define the portion of the index to search, the explain result includes a `CLUSTERED_IXSCAN` stage. The stage includes information about the clustered index key and index bounds.

If the query planner selects a [clustered index](https://www.mongodb.com/docs/reference/method/db.createCollection/#std-label-db.createCollection.clusteredIndex) for a [clustered collection](https://www.mongodb.com/docs/core/clustered-collections/#std-label-clustered-collections) and the query *does not* contain bounds, the query performs an unbounded collection scan and the explain result includes a `COLLSCAN` stage.

The [`notablescan`](https://www.mongodb.com/docs/reference/parameters/#mongodb-parameter-param.notablescan) parameter does not allow unbounded queries that use a clustered index because the queries require a full collection scan.

For more information on execution statistics of collection scans, see [Interpret Explain Plan Results](https://www.mongodb.com/docs/tutorial/analyze-query-plan/).

## Covered Queries

When an index [covers](https://www.mongodb.com/docs/core/query-optimization/#std-label-indexes-covered-queries) a query, MongoDB can both match the query conditions **and** return the results using only the index keys. MongoDB does not need to examine documents from the collection to perform any part of the query.

When an index covers a query, the explain result has an `IXSCAN` stage that is **not** a descendant of a `FETCH` stage.

## `$or` Expression

If MongoDB uses indexes for an [`$or`](https://www.mongodb.com/docs/reference/operator/query/or/#mongodb-query-op.-or) expression, the result will include the `OR` stage with an [`explain.queryPlanner.winningPlan.inputStages`](https://www.mongodb.com/docs/reference/explain-results/#mongodb-data-explain.queryPlanner.winningPlan.inputStages) array that details the indexes; e.g.:
...
When `explain` is run in either [executionStats](https://www.mongodb.com/docs/reference/method/db.collection.explain/#std-label-explain-method-executionStats) or [allPlansExecution](https://www.mongodb.com/docs/reference/method/db.collection.explain/#std-label-explain-method-allPlansExecution) verbosity mode, certain aggregation stages that may spill to disk have additional output. These stages include [`$sort`](https://www.mongodb.com/docs/reference/operator/aggregation/sort/#mongodb-pipeline-pipe.-sort), [`$group`](https://www.mongodb.com/docs/reference/operator/aggregation/group/#mongodb-pipeline-pipe.-group), [`$setWindowFields`](https://www.mongodb.com/docs/reference/operator/aggregation/setWindowFields/#mongodb-pipeline-pipe.-setWindowFields), [`$bucketAuto`](https://www.mongodb.com/docs/reference/operator/aggregation/bucketAuto/#mongodb-pipeline-pipe.-bucketAuto), [`$graphLookup`](https://www.mongodb.com/docs/reference/operator/aggregation/graphLookup/#mongodb-pipeline-pipe.-graphLookup), and [`$lookup`](https://www.mongodb.com/docs/reference/operator/aggregation/lookup/#mongodb-pipeline-pipe.-lookup) (when using the slot-based execution engine).
...
<td headers="Description">
An estimated number of bytes processed in the [`$sort`](https://www.mongodb.com/docs/reference/operator/aggregation/sort/#mongodb-pipeline-pipe.-sort) stage.

</td>
</tr>
<tr>
<td headers="Stage">
[`$sort`](https://www.mongodb.com/docs/reference/operator/aggregation/sort/#mongodb-pipeline-pipe.-sort), [`$group`](https://www.mongodb.com/docs/reference/operator/aggregation/group/#mongodb-pipeline-pipe.-group), [`$setWindowFields`](https://www.mongodb.com/docs/reference/operator/aggregation/setWindowFields/#mongodb-pipeline-pipe.-setWindowFields), [`$bucketAuto`](https://www.mongodb.com/docs/reference/operator/aggregation/bucketAuto/#mongodb-pipeline-pipe.-bucketAuto), [`$graphLookup`](https://www.mongodb.com/docs/reference/operator/aggregation/graphLookup/#mongodb-pipeline-pipe.-graphLookup), [`$lookup`](https://www.mongodb.com/docs/reference/operator/aggregation/lookup/#mongodb-pipeline-pipe.-lookup) (SBE only)
...
<td headers="Description">
An estimate of the number of bytes written to disk before compression.
...
<td headers="Description">
The peak memory usage, in bytes, tracked during stage execution.
...
A hexadecimal string that represents the hash of the [query shape](https://www.mongodb.com/docs/reference/glossary/#std-term-query-shape). For more information, see [Query Shapes](https://www.mongodb.com/docs/core/query-shapes/#std-label-query-shapes).
