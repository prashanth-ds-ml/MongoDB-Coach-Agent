...
Because MongoDB supports dynamic schemas, applications can query against fields whose names are not known in advance or are arbitrary. Wildcard indexes help support these queries. Wildcard indexes are not designed to replace workload-based index planning.
...
You can manage your MongoDB Search and MongoDB Vector Search indexes by using PyMongo. The indexes specify the behavior of the search and which fields to index.
...
You can use the create_search_index() and the create_search_indexes() methods to create MongoDB Search indexes or MongoDB Vector Search indexes.
...
The following code example shows how to create a single MongoDB Vector Search index by using the SearchIndexModel object. Select the Synchronous or Asynchronous tab to see the corresponding code:
...
You can use the create_search_indexes() method to create multiple indexes. These indexes can be MongoDB Search or Vector Search indexes. The `create_search_indexes()` method takes a list of `SearchIndexModel` objects that correspond to each index you want to create.
...
You can use the list_search_indexes() method to get information about the MongoDB Search and Vector Search indexes of a collection.
...
You can use the update_search_index() method to update a MongoDB Search or Vector Search index.
...
You can use the drop_search_index() method to remove a MongoDB Search or MongoDB Vector Search index.
...
MongoDB offers an improved full-text search solution, MongoDB Search. To learn more about MongoDB Search indexes and how to use them, see the MongoDB Search and Vector Search Indexes section of this page.
