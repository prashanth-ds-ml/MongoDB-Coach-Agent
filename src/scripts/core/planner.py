...
def load_md_context(md_files: list) -> str:
    ...
    for fname in md_files:
        fpath = os.path.join(RAW_MD_DIR, fname)
...
def generate_study_calendar(days: int) -> list:
    """
    Spread the 12 syllabus topics across the available days.
...
def mark_topic_complete(user_id: str, topic: str):
    ...
    if topic not in completed:
        ...
        database.update_user_profile(user_id, {"progress": {"completed_topics": completed}})
